...
module.exports = function (mailer, db, options) {
  ...
  return {
    _processReminder: function (reminder) {
        log.debug('_processReminder', reminder)
        // only send email once we safely delete the reminder
        // avoids spamming multiple emails
        //return db.deleteReminder(reminder)
        return P.resolve()
          .then(function () {
            // fetch the account
            return db.account(reminder.uid)
          })
