...
test('_processReminder sends email for unverified emails', function (t) {
  setup()
  ...
  require('../../lib/verification-reminders')(mailer, mockDb)
    ._processReminder({email: TEST_EMAIL, type: REMINDER_TYPE})
})

test('_processReminder sends email for unverified emails', function (t) {
  setup()
  var REMINDER_TYPE = 'second'
  sandbox.stub(mockDb, 'account', function () {
    return TEST_ACCOUNT_RECORD
  })
...
test('_processReminder - does not send email for verified emails', function (t) {
  ...
  require('../../lib/verification-reminders')(mockMailer, mockDb, { log: log })
    ._processReminder({email: 'test@restmail.net'})
})
